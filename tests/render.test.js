  ...
  await test("initial", async (t) => {
    ...
    await t.test("on:updated", async () => {
      await new Promise((resolve, reject) => {
          ...
          on: {
            updated: (e) => {
              const item = stack.find((item) => {
                return (
                  item.property === e.detail?.property &&
        ...
        key = "b";
        el.dispatchEvent(
          new window.CustomEvent("refresh", {
            detail: ["textContent", "dataset.key"],
          }),
        );
      });
    });
        ...
        await new Promise((resolve, reject) => {
          ...
          el = render({
            ...
            on: {
              ...
              changed(e) {
                if (
                  e.detail.attributeName === "my-attr" &&
                  e.detail.newValue === "2" &&
                  e.detail.oldValue === "1"
                ) {
                  resolve();
      ...
      await new Promise((resolve, reject) => {
        ...
        render({
          ...
          on: {
            updated: (e) => {
              if (
                e.detail?.property === "attributes.for" &&
                e.target.getAttribute("for") === "my-input"
              ) {
                resolve();
              }
            },
      ...
      await new Promise((resolve, reject) => {
        ...
        render({
          ...
          on: {
            updated: (e) => {
              if (
                e.detail?.property === "classList" &&
                e.target.className === "a b c"
              ) {
                resolve();
              }
            },
